  ...
  create or replace view `mailshake-analysis-486717`.`SILVER`.`stg_mailshake_activity_opens`
  OPTIONS()
  as 

WITH source AS (
    SELECT * 
    FROM `mailshake-analysis-486717`.`BRONZE`.`activity_opens`
),
...
parsed AS (
    SELECT
        -- Ingestion metadata
        team_id,
        CAST(campaign_id AS INT64) AS campaign_id,  -- ✅ From BRONZE table column (PRIMARY)
        CAST(message_id AS INT64) AS message_id,     -- ✅ From BRONZE table column
        extracted_at,
        source_endpoint,

        -- Primary identifier
        CAST(JSON_VALUE(payload, '$.id') AS INT64) AS open_id,
        ...
        CAST(JSON_VALUE(payload, '$.campaign.id') AS INT64) AS payload_campaign_id,  -- ✅ RENAMED
    ...
    FROM source
    WHERE CAST(JSON_VALUE(payload, '$.isDuplicate') AS BOOL) = FALSE  -- ✅ Filter duplicates
)
