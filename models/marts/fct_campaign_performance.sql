...
activity_metrics AS (
    ...
    FROM campaigns c
    LEFT JOIN {{ ref('stg_mailshake_activity_sent') }} s 
        ON c.campaign_id = s.campaign_id
    LEFT JOIN {{ ref('stg_mailshake_activity_opens') }} o 
        ON c.campaign_id = o.campaign_id
    LEFT JOIN {{ ref('stg_mailshake_activity_clicks') }} cl 
        ON c.campaign_id = cl.campaign_id
    LEFT JOIN {{ ref('stg_mailshake_activity_replies') }} r 
